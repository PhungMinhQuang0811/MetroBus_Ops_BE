package com.vdt.afc_ops_service.security.config;

import com.vdt.afc_ops_service.constant.SecurityConstants;
import com.vdt.afc_ops_service.security.auth.JwtAuthenticationFilter;
import com.vdt.afc_ops_service.security.repository.CustomCsrfTokenRepository;
...
public class SecurityConfig {
    ...
    private void configureAuthorization(AuthorizeHttpRequestsConfigurer<HttpSecurity>.AuthorizationManagerRequestMatcherRegistry auth) {
        String[] publicEndpointPatterns = publicEndpointPatterns();
    ...
    private void configureCsrf(CsrfConfigurer<HttpSecurity> csrf) {
        ...
        String[] publicEndpointPatterns = publicEndpointPatterns();
        if (publicEndpointPatterns.length > 0) {
            csrf.ignoringRequestMatchers(publicEndpointPatterns);
        }
        if (SecurityConstants.ENDPOINT_THIRD_PARTY.length > 0) {
            csrf.ignoringRequestMatchers(SecurityConstants.ENDPOINT_THIRD_PARTY);
        }
    }
