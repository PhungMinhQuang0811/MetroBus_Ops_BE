...
graph LR
    ...
    Guest --> UC01["UC01: Đăng ký mua Thẻ cứng (Guest Checkout)"]
    Guest --> UC27["UC27: Gia hạn thời hạn sử dụng thẻ"]
    Passenger --> UC01
    Passenger --> UC02["UC02: Đăng ký & Đăng nhập OTP di động"]
    Passenger --> UC04["UC04: Đăng xuất tài khoản PWA/Portal"]
    Passenger --> UC07["UC07: Cập nhật hồ sơ cá nhân"]
    Passenger --> UC08["UC08: Số hóa Thẻ cứng thành Thẻ ảo"]
    Passenger --> UC09["UC09: Quét soát vé tự động qua Validator"]
    Passenger --> UC10["UC10: Quản lý ví điện tử, nạp & rút tiền"]
    Passenger --> UC26["UC26: Đăng ký & Phát hành Thẻ ảo trực tiếp"]
    Passenger --> UC27

    %% Internal Staff/Admin Use Cases
    Staff --> UC03["UC03: Đăng nhập Web Portal"]
    Staff --> UC04
    Staff --> UC05["UC05: Thay đổi mật khẩu nội bộ"]
    Staff --> UC06["UC06: Khôi phục mật khẩu nội bộ"]
    Staff --> UC07
    Staff --> UC11["UC11: PSC Giải khóa thẻ kẹt ga"]
    Staff --> UC12["UC12: In thẻ cứng & Cập nhật đơn hàng"]
    Staff --> UC13["UC13: Vận hành ca trực quầy ga"]
    Staff --> UC21["UC21: Khởi tạo lô phôi thẻ cứng mới"]
    Staff --> UC22["UC22: Thu hồi và Vô hiệu hóa thẻ hỏng"]
    ...
    CompManager --> UC07
    CompManager --> UC10
    CompManager --> UC14["UC14: Quản lý Nhân sự & Phân ca trực"]
    CompManager --> UC23["UC23: Quản trị Tuyến ga & lưới trạm"]
    CompManager --> UC24["UC24: Cấu hình Biểu giá tuyến đơn vị"]
    ...
    PlatManager --> UC06
    PlatManager --> UC07
    PlatManager --> UC15["UC15: Khởi tạo Tenant & Cấp tài khoản"]
    PlatManager --> UC16["UC16: Phê duyệt quyết toán doanh nghiệp"]
    PlatManager --> UC25["UC25: Cấu hình Khung giá trần hệ thống"]
    ...
    Admin --> UC06
    Admin --> UC07
    Admin --> UC17["UC17: Ban/Unban khóa tài khoản khẩn cấp"]
    Admin --> UC18["UC18: Cấu hình phân quyền động Dynamic RBAC"]
    Admin --> UC19["UC19: Giám sát kỹ thuật & Tra cứu System Logs"]

    %% System Use Cases
    System --> UC20["UC20: Chạy đối soát Clearing hằng đêm"]
```
...
| :--- | :---: | :--- |
| **Module 1: Xác thực & Tài khoản** | 6 | UC02, UC03, UC04, UC05, UC06, UC07 |
| **Module 2: Thẻ & Vé tháng** | 6 | UC01, UC26, UC08, UC27, UC21, UC22 |
| **Module 3: Soát vé & Vận hành quầy ga** | 4 | UC09, UC11, UC12, UC13 |
| **Module 4: Tài chính & Ví điện tử** | 3 | UC10, UC16, UC20 |
| **Module 5: Quản trị vận hành đơn vị** | 3 | UC14, UC23, UC24 |
| **Module 6: Quản trị nền tảng hệ thống** | 2 | UC15, UC25 |
| **Module 7: Giám sát, Bảo mật & Phân quyền** | 3 | UC17, UC18, UC19 |
...
### UC02: Đăng ký & Đăng nhập bằng OTP Số điện thoại (Passenger Mobile PWA)
...
5. **Bước 5:** Backend xác thực mã OTP:
   ...
   * *If là SĐT mới chưa từng đăng ký:* Hệ thống tự động tạo mới tài khoản (`accounts`) với quyền `ROLE_PASSENGER` (gán cứng ở backend để bảo mật), tự động tạo hồ sơ cá nhân (`account_profile`) rỗng và ví điện tử cá nhân (`wallet` có số dư = 0đ).
6. **Bước 6:** Hành khách đăng nhập thành công, chuyển hướng vào màn hình Dashboard chính của PWA.

#### Luồng ngoại lệ (Exception Flows):
*   **Exc 2a - Nhập sai mã OTP:** Khách nhập sai mã OTP hoặc OTP bị quá hạn 2 phút, hệ thống báo đỏ từ chối và yêu cầu nhập lại hoặc gửi lại mã khác.

---

### UC03: Đăng nhập tài khoản nội bộ (Username/Password - Admin, Staff, Managers)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 3a - Sai thông tin đăng nhập hoặc tài khoản bị khóa:** Hệ thống báo lỗi "Tên đăng nhập hoặc mật khẩu không chính xác" hoặc "Tài khoản của bạn đã bị tạm khóa, vui lòng liên hệ Admin". Logic đăng nhập bị từ chối.

---

### UC04: Đăng xuất tài khoản (Logout - Tất cả Actors)
...
### UC05: Thay đổi mật khẩu nội bộ (Change Password - Admin, Staff, Managers)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 5a - Nhập sai mật khẩu cũ:** Hệ thống báo lỗi "Mật khẩu cũ không chính xác" và chặn thay đổi mật khẩu.

---

### UC06: Khôi phục mật khẩu nội bộ (Forgot/Reset Password - Admin, Staff, Managers)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 6a - Nhập email không tồn tại:** Hệ thống báo lỗi "Email không tồn tại trong hệ thống".
*   **Exc 6b - Token quá hạn:** Nếu người dùng click link sau 15 phút, hệ thống báo lỗi "Liên kết khôi phục mật khẩu đã quá hạn. Vui lòng thực hiện gửi lại yêu cầu!".

---

### UC07: Cập nhật hồ sơ cá nhân (Passenger Mobile PWA & Web Portal)
...
| **Hậu điều kiện** | Thông tin hồ sơ cá nhân tiêu chuẩn được cập nhật thành công trong bảng `account_profile` của DB. |
...
5. **Bước 5:** Backend thực hiện cập nhật các trường tương ứng trong bảng `account_profile` thuộc CSDL `auth_db`.
...
*   **Alt 7a - Hoàn thiện thông tin KYC & Xác thực Email kích hoạt Ví/Thẻ ảo (Dành riêng cho Passenger):**
    ...
    *   Backend đối khớp mã OTP gửi lên:
        * *Nếu trùng khớp:* Thực hiện Database Transaction ACID cập nhật `personal_id` và ảnh chân dung vào bảng `account_profile`, đồng thời cập nhật địa chỉ email và đặt trạng thái **`is_email_verified = TRUE`** trong bảng `accounts`.
        * Hệ thống tự động kích hoạt Ví điện tử cá nhân của khách hàng (sẵn sàng thực hiện nạp/rút tiền) và mở khóa tính năng số hóa thẻ ảo (UC08).
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 7b - Nhập sai mã OTP xác thực email (Alt 7a):** Hành khách nhập sai OTP hoặc OTP quá hạn 5 phút, hệ thống báo đỏ lỗi "Mã xác thực OTP không chính xác hoặc đã hết hiệu lực", không lưu email và không kích hoạt trạng thái KYC của Passenger. Khách hàng phải nhấn gửi lại mã xác thực khác.
*   **Exc 7c - Lỗi quét eKYC OCR thất bại (Alt 7a - Phương thức 2):** Ảnh chụp CCCD bị mờ, lóa sáng, mất góc hoặc không đúng định dạng. Hệ thống hiển thị thông báo lỗi: "Không thể nhận diện được thông tin trên ảnh chụp CCCD. Vui lòng chụp lại ảnh rõ nét hơn hoặc chuyển sang phương thức Nhập thủ công!".
...
*Toàn bộ nghiệp vụ liên quan đến thẻ vật lý và thẻ ảo: đăng ký mua thẻ, phát hành thẻ ảo, số hóa thẻ cứng, gia hạn, thu hồi và quản lý kho phôi thẻ.*

### UC01: Đăng ký mua Thẻ cứng trực tuyến (Web Portal - Guest Checkout)
...
| :--- | :--- |
| **Mã Use Case** | **UC01** |
| **Tác nhân chính** | Khách vãng lai / Guest, Hành khách / Passenger |
...
7. **Bước 7:** Hệ thống khởi tạo đơn hàng với trạng thái `PENDING_PAYMENT`, tự động tính tổng số tiền (Phí làm thẻ + Phí vé tháng đăng ký + Phí ship nếu có) và chuyển hướng khách hàng sang cổng Sandbox VNPay. (Cột `user_id` trong bảng `orders` nhận giá trị `NULL` nếu mua ẩn danh).
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 1a - Hủy thanh toán:** Khách hàng nhấn hủy hoặc không thanh toán trong 15 phút, hệ thống tự động đổi `order_status = 'CANCELLED'`.

---

### UC26: Đăng ký và Phát hành Thẻ ảo trực tiếp (Passenger Mobile PWA)
*   **Mô tả:** Hành khách (`PASSENGER`) sau khi đăng ký tài khoản bằng OTP SĐT (UC02) và hoàn thiện KYC/xác thực email (UC07 - Alt 7a), thực hiện quy trình đăng ký phát hành thẻ ảo đi kèm chọn mua gói vé tháng lần đầu ngay trên ứng dụng di động Mobile PWA để kích hoạt thẻ và sử dụng đi lại tức thời mà không cần qua thẻ cứng vật lý.

| Thuộc tính | Chi tiết đặc tả |
| :--- | :--- |
| **Mã Use Case** | **UC26** |
| **Tác nhân chính** | Hành khách / Passenger |
| **Tiền điều kiện** | Hành khách đã đăng nhập thành công vào Mobile PWA, tài khoản đã được hoàn thiện thông tin KYC (`is_kyc_verified = TRUE`) và đã xác thực địaิ chỉ email cá nhân (`is_email_verified = TRUE`) thông qua quy trình tại UC07 - Alt 7a. Hành khách chưa sở hữu thẻ ảo hoạt động nào trên tài khoản. |
| **Hậu điều kiện** | Hệ thống khởi tạo và phát hành thành công một thẻ ảo điện tử mới (`card_medium = 'VIRTUAL'`) liên kết trực tiếp với ví điện tử của khách hàng. Khởi tạo và kích hoạt thành công một gói vé tháng/chu kỳ cước tương ứng trong bảng `passes`. Trạng thái thẻ ảo và gói vé được đặt là `ACTIVE`. Thông tin thẻ ảo và thời hạn sử dụng vé tháng được hiển thị trực quan trên giao diện PWA. |
...
6. **Bước 6:** Backend tiếp nhận yêu cầu, thực hiện chuỗi xác thực nghiệp vụ (Business Rule Validation):
   ...
   * Truy vấn CSDL `user_db`, xác nhận `is_kyc_verified = TRUE` và `is_email_verified = TRUE`.
   * Truy vấn CSDL `ticket_db`, xác nhận hành khách chưa có bản ghi thẻ ảo nào ở trạng thái `ACTIVE` hoặc `PENDING`.
   * Truy vấn CSDL `user_db` bảng ví, xác nhận số dư ví điện tử của hành khách đủ tiền thanh toán cước gói vé tháng đã chọn: `wallets.balance >= selected_pass_price` (trong đó `selected_pass_price` là giá tiền của gói vé tháng đã lựa chọn, ví dụ: 200,000 VNĐ đối với gói liên tuyến).
7. **Bước 7:** Backend khởi chạy một giao dịch CSDL ACID (Database Transaction) để thực hiện trừ tiền và phát hành thẻ:
   * Khóa dòng (Row Locking) bản ghi ví điện tử `wallets` của hành khách để khấu trừ tiền mua vé tháng: `wallets.balance = wallets.balance - selected_pass_price`.
   * Sinh mã định danh thẻ ngẫu nhiên toàn cục `card_uid` duy nhất (độ dài 16 ký tự hexa hoặc định dạng UUID v4 rút gọn).
   * Tạo bản ghi mới trong bảng `cards` với cấu hình: `card_uid`, `owner_id = passenger_id`, `card_medium = 'VIRTUAL'`, `status = 'ACTIVE'` (lưu ý: loại bỏ hoàn toàn trường `balance` đối với thẻ ảo do hệ thống soát vé chặng vé tháng chỉ cần kiểm tra trạng thái `cards.status = 'ACTIVE'` và thời hạn gói vé `passes.end_date` mà không cần lưu trữ số dư độc lập trên thẻ ảo), `created_at = CURRENT_TIMESTAMP`.
   * Tạo bản ghi vé tháng mới trong bảng `passes` (hoặc `card_passes`): `card_uid`, `pass_type = 'ALL_LINES'` (hoặc gói chặng tương ứng), `start_date = CURRENT_TIMESTAMP`, `end_date = CURRENT_TIMESTAMP + 30 days` (30 ngày sử dụng kể từ thời điểm phát hành), `status = 'ACTIVE'`.
   * Tự động tạo khoá bí mật đối xứng `qr_secret_key` dùng riêng để sinh mã QR động TOTP (Time-based One-Time Password) bảo mật tại thiết bị di động của khách hàng khi quét qua cổng validator, lưu trữ khoá bí mật này vào bảng `card_security_configs`.
   * Ghi nhận giao dịch mua vé tháng thành công trong bảng `transactions`: `transaction_type = 'PURCHASE'`, `payment_method = 'WALLET'`, `amount = selected_pass_price`, `status = 'SUCCESS'`, `wallet_id = ID_Ví_Khách`, `card_uid = card_uid`.
...
9. **Bước 9:** Trên màn hình Mobile PWA của hành khách hiển thị hiệu ứng chúc mừng sinh động kèm hình ảnh thẻ ảo 3D, mã số thẻ `card_uid` che ẩn 8 ký tự giữa, thời hạn sử dụng vé tháng hiển thị rõ ràng (`Hạn dùng: Đến 23:59 ngày X/Y/Z`), và hiển thị mã QR động TOTP biến đổi liên tục mỗi 30 giây để hành khách soát vé đi tàu/xe bus tức thời (UC09).
...
#### Luồng rẽ nhánh (Alternative Flows):
*   **Alt 26a - Kích hoạt khóa mã QR động ngoại tuyến (Offline QR dynamic key):**
    *   Bước Alt 26a.1: Tại Bước 9, PWA tải và lưu khóa bí mật `qr_secret_key` cùng cấu hình thuật toán sinh mã vào bộ nhớ cục bộ an toàn của ứng dụng di động (HTML5 Web Storage / IndexedDB).
    *   Bước Alt 26a.2: Khi hành khách đi qua ga tàu nhưng thiết bị mất kết nối Internet (mất mạng 3G/4G/Wifi), ứng dụng Mobile PWA vẫn tự động sử dụng thư viện Javascript chạy ngầm lấy thời gian hệ thống nội bộ của thiết bị kết hợp với khóa bí mật `qr_secret_key` để sinh mã QR động TOTP offline hợp lệ. Cổng soát vé tự động Gate Validator (UC09) quét mã này, tự tính toán đối sánh và mở cửa bình thường mà không cần PWA phải online.
*   **Alt 26b - Thanh toán cước gói vé tháng lần đầu qua VNPay Sandbox (Khi số dư ví không đủ hoặc khách muốn thanh toán trực tuyến):**
    *   Bước Alt 26b.1: Tại Bước 5, hành khách lựa chọn phương thức thanh toán "Thanh toán trực tuyến (VNPay Gateway)".
    *   Bước Alt 26b.2: Hệ thống sinh phiên thanh toán với trạng thái `PENDING_PAYMENT` và chuyển hướng hành khách sang giao diện VNPay Sandbox.
    *   Bước Alt 26b.3: Hành khách thực hiện thanh toán giả lập thành công. VNPay Merchant gửi tín hiệu IPN Callback xác nhận, backend khởi chạy giao dịch ACID đồng bộ:
        ...
        *   Tạo bản ghi vé tháng mới trong bảng `passes` ở trạng thái `ACTIVE` có thời hạn 30 ngày.
        *   Tạo khóa `qr_secret_key` sinh QR động TOTP offline.
        *   Cập nhật trạng thái transaction `status = 'SUCCESS'`, `payment_method = 'VNPAY'`.
    *   Bước Alt 26b.4: Giao diện Mobile PWA tự động cập nhật thời gian thực, hiển thị popup chúc mừng và mã QR động đã được kích hoạt sử dụng ngay lập tức tương tự như Bước 9.

#### Luồng ngoại lệ (Exception Flows):
*   **Exc 26a - Tài khoản chưa thực hiện KYC hoặc chưa xác thực Email:**
    ...
    *   Giao diện Mobile PWA hiển thị thông báo: "Bạn chưa hoàn thiện hồ sơ KYC hoặc xác thực Email cá nhân. Thẻ ảo chỉ được phát hành cho tài khoản định danh để đảm bảo an toàn giao dịch. Vui lòng hoàn thành KYC ngay!". Đồng thời hiển thị nút shortcut dẫn thẳng khách hàng sang phân hệ KYC của UC07 - Alt 7a.
*   **Exc 26b - Đã sở hữu thẻ ảo đang hoạt động:**
...
*   **Exc 26c - Thất bại khi sinh mã card_uid duy nhất (Trùng lặp key):**
    ...
    *   Giao diện hiển thị lỗi: "Hệ thống đang bận xử lý yêu cầu phát hành thẻ ảo. Vui lòng thử lại sau ít phút!". snapshot lỗi được đẩy sang MongoDB `system_errors` kèm cảnh báo hệ thống (UC19).
*   **Exc 26d - Số dư ví của Passenger không đủ và từ chối thanh toán:**
...
### UC08: Số hóa Thẻ cứng vật lý thành Thẻ điện tử ảo (Mobile PWA)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 8a - Thông tin đối khớp CCCD thất bại:** Số CCCD nhập vào không khớp với dữ liệu đăng ký mua thẻ cứng ban đầu. Hệ thống hiển thị thông báo lỗi: "Thông tin xác thực CCCD không khớp với đơn hàng đăng ký mua thẻ cứng. Vui lòng kiểm tra lại hoặc liên hệ Quầy dịch vụ ga để được hỗ trợ". Logic số hóa bị chặn đứng.
*   **Exc 8b - Thẻ cứng vật lý đã bị số hóa trước đó:** Nếu trạng thái thẻ cứng gốc là `VIRTUALIZED`, hệ thống từ chối xử lý và báo lỗi: "Thẻ vật lý này đã được số hóa sang thẻ ảo trước đó. Không thể thực hiện lại thao tác!".

---

### UC27: Gia hạn thời hạn sử dụng thẻ (Passenger Mobile PWA & Web Portal)
*   **Mô tả:** Hành khách (`PASSENGER`) hoặc Khách vãng lai (`GUEST`) thực hiện gia hạn thời hạn sử dụng của thẻ cứng vật lý hoặc thẻ ảo để tiếp tục sử dụng dịch vụ vé tháng đi lại. Đối với Hành khách (đã đăng nhập PWA), thanh toán được thực hiện bằng cách khấu trừ trực tiếp từ số dư Ví điện tử cá nhân. Đối với Khách vãng lai (không đăng nhập Web Portal), thanh toán được thực hiện trực tuyến qua cổng VNPay Sandbox.
...
| **Hậu điều kiện** | Thời hạn hết hạn của thẻ (`cards.expired_at`) được kéo dài thêm 30 ngày (hoặc chu kỳ cước tháng tương ứng kể từ thời điểm hết hạn cũ hoặc thời điểm hiện tại). Ghi nhận giao dịch thành công trong bảng `transactions` và lưu lịch sử doanh thu gia hạn thẻ. |
| **Tác nhân kích hoạt** | Người dùng nhấn chọn nút "Gia hạn thẻ" trên ứng dụng Mobile PWA hoặc Web Portal công cộng. |

#### Luồng xử lý chính (Basic Flow):

##### Option A: Hành khách (Passenger) gia hạn qua ứng dụng di động Mobile PWA (Dùng số dư Ví điện tử)
1. **Bước A1.1:** Hành khách truy cập phân hệ "Ví & Thẻ của tôi" trên ứng dụng di động Passenger Mobile PWA, danh sách thẻ cứng và thẻ ảo của hành khách hiển thị đầy đủ kèm theo thời hạn hết hạn hiện tại (`expired_at`).
2. **Bước A1.2:** Hành khách chọn một thẻ bất kỳ đang hoạt động hoặc đã hết hạn và nhấn nút **"Gia hạn thẻ"**.
3. **Bước A1.3:** Hệ thống kiểm tra thông tin cước và hiển thị form gia hạn: hiển thị thông tin thẻ, lựa chọn số chu kỳ gia hạn (mặc định 1 tháng / 30 ngày cước với số tiền quy định, gọi là cước gia hạn `renewal_fee`, ví dụ: 200,000 VNĐ) và số dư ví điện tử hiện tại của hành khách.
...
5. **Bước A1.5:** Backend nhận yêu cầu gia hạn, xác thực token JWT và bắt đầu thực thi một giao dịch CSDL ACID (Database Transaction):
   * Khóa dòng (Row Locking) bản ghi ví điện tử `wallets` của hành khách để kiểm tra số dư: xác nhận `wallets.balance >= renewal_fee`.
   * Khấu trừ số dư ví của hành khách: `wallets.balance = wallets.balance - renewal_fee`.
   * Truy vấn thông tin thẻ hiện tại từ bảng `cards` theo `card_uid`. Tính toán thời điểm hết hạn mới:
     $$\text{expired\_at\_new} = \max(\text{cards.expired\_at}, \text{CURRENT\_TIMESTAMP}) + 30\text{ ngày}$$
   * Cập nhật bản ghi thẻ trong bảng `cards`: `cards.expired_at = expired_at_new`, trạng thái thẻ tự động chuyển thành `ACTIVE` (nếu trước đó thẻ ở trạng thái `EXPIRED`).
   * Ghi nhận giao dịch thành công trong bảng `transactions`: `transaction_type = 'RENEWAL'`, `payment_method = 'WALLET'`, `amount = renewal_fee`, `status = 'SUCCESS'`, `wallet_id = ID_Ví_Khách`, `card_uid = card_uid`.
6. **Bước A1.6:** Hệ thống gửi Email thông báo gia hạn thẻ thành công tới email cá nhân của hành khách.
7. **Bước A1.7:** Màn hình Mobile PWA tự động hiển thị popup chúc mừng gia hạn thẻ thành công kèm thời hạn sử dụng mới cập nhật trực quan tức thời. snapshot giao dịch được lưu vết vào MongoDB `audit_logs`.

##### Option B: Khách vãng lai (Guest) gia hạn thẻ cứng vật lý qua Web Portal công cộng (Dùng VNPay Sandbox)
1. **Bước B1.1:** Khách vãng lai truy cập Cổng thông tin công cộng Web Portal và chọn tính năng "Gia hạn thẻ cứng trực tuyến".
...
5. **Bước B1.5:** Web Portal hiển thị thông tin tóm tắt của thẻ cứng (bao gồm Họ tên chủ thẻ được che ẩn ký tự như `Ngu*** Van A`, Loại thẻ, và Ngày hết hạn hiện tại) kèm theo mức cước gia hạn 30 ngày (gọi là cước gia hạn `renewal_fee`, ví dụ: 200,000 VNĐ).
...
9. **Bước B1.9:** Backend tiếp nhận tín hiệu IPN Callback xác thực chữ ký số từ VNPay xác nhận giao dịch thanh toán thành công, lập tức khởi chạy một giao dịch CSDL ACID (Database Transaction) đồng bộ:
   * Cập nhật thời hạn sử dụng mới cho thẻ cứng vật lý:
     $$\text{cards.expired\_at} = \max(\text{cards.expired\_at}, \text{CURRENT\_TIMESTAMP}) + 30\text{ ngày}$$
     (Nếu trạng thái thẻ đang là `EXPIRED` thì chuyển về `ACTIVE`).
   * Ghi nhận giao dịch thành công trong bảng `transactions`: Cập nhật trạng thái `status = 'SUCCESS'`, `payment_method = 'VNPAY'`, ghi nhận mã giao dịch VNPay `payment_transaction_id`.
...
11. **Bước B1.11:** Giao diện Web Portal tự động nhận được callback cập nhật thời gian thực, hiển thị thông báo "Gia hạn thẻ cứng thành công!" kèm biên lai điện tử in thời hạn hết hạn mới của thẻ.

#### Luồng rẽ nhánh (Alternative Flows):
*   **Alt 27a - Passenger gia hạn thẻ của mình bằng thanh toán VNPay Sandbox (Khi số dư ví không đủ):**
    *   Bước Alt 27a.1: Tại Bước A1.3, hành khách thấy số dư ví điện tử của mình không đủ thanh toán. Hành khách chọn phương thức thanh toán "Thanh toán trực tuyến (VNPay Gateway)" thay vì Ví điện tử.
    *   Bước Alt 27a.2: Hệ thống chuyển hướng hành khách sang giao diện VNPay Sandbox tương tự các bước B1.7 - B1.9 của Option B. Sau khi thanh toán thành công, thẻ của hành khách được cập nhật gia hạn thêm 30 ngày và giao dịch được ghi nhận dưới dạng `payment_method = 'VNPAY'`, `wallet_id = ID_Ví_Khách`.

#### Luồng ngoại lệ (Exception Flows):
*   **Exc 27a - Tra cứu không thấy mã thẻ hoặc thẻ không hợp lệ (Khách vãng lai):**
    *   Tại Bước B1.4, backend truy vấn CSDL nhưng không thấy mã `card_uid` tồn tại, hoặc thẻ đang có trạng thái `LOCKED` (bị khóa do báo mất/vi phạm an ninh).
    *   Backend trả về lỗi `CardNotValidException`. Giao diện Web Portal hiển thị cảnh báo lỗi màu đỏ: "Mã số thẻ không tồn tại trên hệ thống hoặc đã bị khóa. Vui lòng kiểm tra lại hoặc liên hệ quầy dịch vụ PSC để được hỗ trợ!".
*   **Exc 27b - Số dư ví của Passenger không đủ và từ chối thanh toán:**
...
#### Luồng rẽ nhánh (Alternative Flows):
* **Alt 27c - Hành khách tra cứu lịch sử vé đã mua và hóa đơn (Ticket Purchase History Query):**
    ...
    * Backend giải mã token lấy `account_id`, thực hiện truy vấn (Query) vào bảng dữ liệu `transactions` kết hợp thông tin cấu hình từ bảng `passes` (Vé tháng) hoặc `tickets` (Vé lượt lẻ) để lấy lịch sử mua.
...
### UC21: Khởi tạo lô phôi thẻ cứng vật lý mới (Staff Portal)
*   **Mô tả:** Nhân viên ga (`STAFF`) thuộc bộ phận kho thẻ đăng nhập Portal, thực hiện quét mã định danh phôi thẻ cơ học thực tế (qua thiết bị đọc RFID hoặc đầu quét chuyên dụng) để khởi tạo hàng loạt thực thể phôi thẻ cứng vật lý mới vào CSDL hệ thống, chuẩn bị đưa vào phân phối.
...
2. **Bước 2:** Hệ thống hiển thị giao diện nạp phôi thẻ, cung cấp hai tùy chọn: Nhập mã đơn lẻ hoặc Quét nạp hàng loạt (Bulk upload qua file CSV/đầu đọc).
3. **Bước 3:** Nhân viên thực hiện quét lô phôi thẻ cơ học bằng đầu đọc. Đầu đọc ghi nhận danh sách mã định danh độc nhất của phôi (`card_uid` dạng Hexadecimal, ví dụ: `1A2B3C4D`).
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 21a - Trùng lặp mã thẻ `card_uid`:** Backend phát hiện một hoặc nhiều `card_uid` trong lô nạp đã tồn tại trong database. Hệ thống tự động tách lọc danh sách bị trùng, chặn nạp các thẻ này, vẫn nạp các thẻ hợp lệ khác và trả về danh sách cảnh báo mã trùng trên màn hình Portal để nhân viên ga kiểm tra lại phôi thẻ.

---

### UC22: Thu hồi và Vô hiệu hóa thẻ vật lý hỏng/lỗi (Staff Portal)
*   **Mô tả:** Khi hành khách trả lại thẻ vật lý bị nứt, gãy, lỗi chip không thể quét tại cổng Validator, nhân viên ga (`STAFF`) thực hiện tra cứu mã thẻ trên Portal, vô hiệu hóa thẻ hỏng vĩnh viễn và thực hiện thu hồi cơ học để tiêu hủy.
...
| **Hậu điều kiện** | Trạng thái của thẻ vật lý bị đổi sang `EXPIRED` hoặc `LOCKED` vĩnh viễn trên database, các gói vé chu kỳ còn hạn (nếu có) bị đóng băng để chuẩn bị di trú. |
| **Tác nhân kích hoạt** | Nhân viên nhấn nút "Thu hồi thẻ hỏng" sau khi kiểm tra thẻ lỗi cơ lý học của khách. |

#### Luồng xử lý chính (Basic Flow):
1. **Bước 1:** Nhân viên ga truy cập phân hệ "Thu hồi & Xử lý thẻ lỗi" trên Portal.
2. **Bước 2:** Nhân viên ga thực hiện quét thẻ hỏng lên đầu đọc tại quầy để lấy mã `card_uid` (hoặc nhập thủ công mã thẻ do khách cung cấp).
...
4. **Bước 4:** Nhân viên nhấn nút **"Thu hồi & Vô hiệu hóa thẻ"**, chọn lý do thu hồi (Hỏng vật lý, Lỗi chip đầu đọc) và giữ lại thẻ cơ học của khách.
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 22a - Thẻ đã bị vô hiệu hóa từ trước:** Nếu thẻ tra cứu có trạng thái khác `ACTIVE` (ví dụ: `VIRTUALIZED` hoặc `EXPIRED`), hệ thống từ chối xử lý và hiển thị thông báo lỗi: "Thẻ này không ở trạng thái hoạt động bình thường, không thể thực hiện thu hồi lỗi!".
...
### UC09: Quét soát vé tự động qua cổng Gate Validator (Metro & Bus)
...
#### Luồng rẽ nhánh (Alternative Flows):
*   **Alt 9a - Soát vé trên Phân hệ xe Bus (Check-in 1 chiều):**
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 9b - Quét đúp (Anti-passback):** Hành khách quẹt thẻ liên tiếp trong vòng 60 giây tại cùng một trạm kiểm soát. Validator báo đèn đỏ từ chối và hiển thị cảnh báo: "Thao tác quá nhanh. Vui lòng đợi 60 giây trước khi quét lại!".
*   **Exc 9c - Vé lượt đi quá ga đăng ký (Over-riding ở Metro):** Hệ thống phát hiện cự ly thực tế khách đi lớn hơn cự ly ga đã mua trước trên vé lẻ (`actual_delta_S > paid_delta_S`). Hệ thống chặn lại, rào chắn đóng chặt, Validator báo đèn đỏ và hiển thị cảnh báo: *"Vé không hợp lệ cho ga ra này. Vui lòng di chuyển tới Quầy dịch vụ ga để nạp chênh lệch"*. Khách hàng phải di chuyển qua PSC để xử lý thủ công (UC11).
*   **Exc 9d - Quẹt thẻ bị khóa (LOCKED do lỗi quên check-out chặng trước):** Hệ thống phát hiện thẻ ảo của khách đang ở trạng thái `LOCKED`. Validator báo đèn đỏ và hiển thị cảnh báo: *"Thẻ bị tạm khóa do lỗi hành trình trước. Vui lòng liên hệ Quầy dịch vụ ga để được mở khóa"*.

#### Luồng rẽ nhánh (Alternative Flows):
* **Alt 9e - Hành khách tra cứu lịch sử chuyến đi và vé đã mua (History Query):**
...
### UC11: PSC Xử lý sự cố và Giải khóa Thẻ kẹt ga (Portal - Quầy dịch vụ PSC)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 11a - Ca trực của nhân viên đã bị đóng:** Backend phát hiện ca trực `shift_id` của nhân viên này đã đóng (`status = 'CLOSED'`). Hệ thống từ chối thực hiện giao dịch và báo lỗi: "Ca trực hiện tại đã kết thúc. Vui lòng mở ca trực mới trước khi thực hiện giao dịch tài chính!".

---

### UC12: In thẻ cứng vật lý và Cập nhật trạng thái đơn hàng (Portal - Ca trực)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 12a - Khách hàng từ chối nhận thẻ do in lỗi thông tin:** Nhân viên ga hủy cập nhật `COMPLETED`, chuyển trạng thái đơn hàng ngược về `PRINTING` trong hàng chờ in lại để chỉnh sửa in thẻ mới miễn phí cho khách.

---

### UC13: Vận hành ca trực quầy ga và Đối chiếu két tiền mặt (Staff Portal)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 13a - Phát hiện lệch tiền két thực tế với báo cáo hệ thống:** Khi thủ quỹ kiểm đếm két tiền thực tế phát hiện lệch (thừa hoặc thiếu) so với con số trên báo cáo hệ thống. Nhân viên ga phải chọn "Ghi nhận chênh lệch két", nhập số tiền mặt đếm thực tế và điền lý do giải trình bắt buộc. Hệ thống cập nhật trường `actual_cash_counted` và `discrepancy_reason` vào bảng `staff_shifts`, lưu vết biên bản đối chiếu ca trực và gửi báo cáo chênh lệch đến Company Manager phê duyệt ca trực.
...
*Quản lý ví điện tử hành khách và ví doanh nghiệp: nạp tiền, rút tiền, phê duyệt quyết toán và đối soát doanh thu clearing hằng đêm.*

### UC10: Quản lý ví điện tử, nạp và rút tiền (E-Wallet Management - Sepay Integration)
*   **Mô tả:** Phân hệ quản lý tài chính ví điện tử PFC. Hành khách thực hiện nạp tiền tự động vào ví điện tử cá nhân thông qua giải pháp **Sepay.vn** (quét mã VietQR động tự động nhận biến động số dư qua Webhook thời gian thực) hoặc qua cổng VNPay Sandbox. Hành khách cũng có thể rút khoản tiền dư từ ví cá nhân về tài khoản ngân hàng thực tế của mình. Quản lý công ty vận hành (`COMPANY_MANAGER`) thực hiện kiểm tra số dư Ví doanh nghiệp (`Operator Wallet`) và gửi đề xuất yêu cầu rút tiền quyết toán dòng tiền về tài khoản ngân hàng thực tế của doanh nghiệp.

| Thuộc tính | Chi tiết đặc tả |
| :--- | :--- |
| **Mã Use Case** | **UC10** |
| **Tác nhân chính** | Hành khách / Passenger (nạp & rút ví cá nhân), Quản lý đơn vị / Company Manager (gửi yêu cầu rút ví doanh nghiệp) |
| **Tiền điều kiện** | Người dùng đã đăng nhập thành công. Hành khách phải hoàn tất hoàn thiện hồ sơ và xác thực email (UC07) trước khi thực hiện các giao dịch nạp/rút tiền ví điện tử để bảo mật danh tính. |
| **Hậu điều kiện** | **Luồng nạp:** Số dư ví được cập nhật cộng tiền tức thời, tạo giao dịch thành công. **Luồng rút tự động (< 5tr):** Ví bị trừ tiền tức thì, hệ thống ngân hàng chuyển khoản thực tế qua NAPAS 24/7 tức thời, giao dịch thành `SUCCESS`. **Luồng rút lớn/doanh nghiệp (>= 5tr):** Tiền ví bị khóa, tạo lệnh giao dịch ở trạng thái `PENDING_SETTLEMENT` chờ Platform Manager duyệt (UC16). |
| **Tác nhân kích hoạt** | Người dùng chọn chức năng Ví của tôi trên PWA/Portal và thực hiện giao dịch nạp/rút tiền. |

#### Luồng xử lý chính (Basic Flow):

##### Phân đoạn A: Các luồng Nạp tiền vào Ví điện tử (Top-up Flows)

###### Option A1: Hành khách nạp tiền ví cá nhân trực tuyến qua VietQR tự động (Sepay Integration)
...
3. **Bước A1.3:** Backend gọi API của **Sepay.vn** để sinh mã **VietQR động** có chứa:
   ...
   * Nội dung chuyển khoản (Syntax) được sinh tự động độc nhất: **`PFC TOPUP [wallet_id]`** (với `wallet_id` là ID ví điện tử cá nhân của chính hành khách này).
...
6. **Bước A1.6:** Endpoint tiếp nhận Webhook và xử lý nghiệp vụ tự động:
   ...
   * Đọc và bóc tách chuỗi nội dung chuyển khoản để lấy mã định danh ví `wallet_id` sau cụm từ khóa `PFC TOPUP`.
   * Khởi chạy giao dịch ACID đồng bộ:
     * Cộng tiền thực nhận vào tài khoản ví của khách hàng: `wallet.balance = balance + amount`.
     * Tạo một bản ghi giao dịch trong bảng `transactions` thuộc `ticket_db` với trạng thái `status = 'SUCCESS'`, `transaction_type = 'TOP_UP'`, `payment_method = 'SEPAY'`, `amount = số tiền nạp`, `wallet_id = wallet_id`.
...
###### Option A2: Hành khách nạp tiền ví cá nhân trực tuyến qua VNPay Sandbox
...
11. **Bước A2.4:** Backend nhận IPN callback xác nhận thanh toán thành công, thực thi giao dịch ACID:
    * Cộng tiền vào tài khoản ví của khách hàng: `wallet.balance = balance + amount`.
    * Ghi nhận giao dịch trong bảng `transactions` với `transaction_type = 'TOP_UP'`, `payment_method = 'VNPAY'`, `amount = số tiền nạp`, `status = 'SUCCESS'`, `wallet_id = ID_Ví_Khách`.
12. **Bước A2.5:** Màn hình PWA tự động cập nhật số dư mới tức thời kèm thông báo nạp tiền thành công.

##### Phân đoạn B: Các luồng Rút tiền khỏi Ví điện tử (Withdrawal Flows)

###### Option B1: Hành khách rút tiền từ Ví điện tử cá nhân (Dual-Mode Payout)
13. **Bước B1.1:** Hành khách mở ứng dụng Mobile PWA, vào mục "Ví của tôi" và chọn nút **"Rút tiền khỏi ví"**.
14. **Bước B1.2:** Hành khách nhập số tiền cần rút (tối thiểu 5,000 VNĐ và nhỏ hơn hoặc bằng số dư ví khả dụng hiện tại) và chọn ngân hàng nhận cùng số tài khoản ngân hàng thụ hưởng cá nhân.
15. **Bước B1.3:** Hành khách nhấn nút **"Xác nhận rút tiền"**.
16. **Bước B1.4:** Backend tiếp nhận yêu cầu và khởi chạy giao dịch ACID kiểm duyệt ban đầu:
    * So khớp số tiền yêu cầu rút nhỏ hơn hoặc bằng số dư ví khả dụng hiện hành của khách.
    * Thực thi cơ chế phân lớp xử lý **Phân lớp giải ngân tự động (Dual-Mode Payout)**:
      * **Trường hợp B1.4.1 - Giải ngân tự động dưới hạn mức (Số tiền rút nhỏ < 5,000,000 VNĐ):**
        * Backend trừ trực tiếp số dư ví của khách hàng tức thì: `wallet.balance = balance - amount`.
        * Backend gọi Outbound Payout API bảo mật (tích hợp cổng chuyển tiền tự động Napas 24/7 của Sepay/ngân hàng) thực hiện lệnh chuyển khoản ngân hàng thực tế trực tiếp vào tài khoản ngân hàng cá nhân của khách hàng.
        * Nhận callback chuyển tiền thành công, hệ thống ghi nhận bản ghi giao dịch trong bảng `transactions`: `transaction_type = 'WITHDRAW'`, `payment_method = 'SEPAY_PAYOUT'`, `amount = số tiền rút`, `status = 'SUCCESS'`, `wallet_id = ID_Ví_Khách`. Khách nhận tiền ngân hàng báo có tức khắc trong vòng 30 giây.
      * **Trường hợp B1.4.2 - Treo quỹ chờ kiểm duyệt thủ công (Số tiền rút lớn >= 5,000,000 VNĐ):**
        * Backend trừ tạm thời số tiền rút khỏi số dư ví khả dụng của khách và đưa vào hàng chờ khóa tạm để bảo vệ an toàn: `wallet.balance = balance - amount`.
        * Tạo bản ghi giao dịch trong bảng `transactions` bên `ticket_db`: `transaction_type = 'WITHDRAW'`, `amount = amount`, `status = 'PENDING_SETTLEMENT'`, `wallet_id = ID_Ví_Khách`.
        * Đưa giao dịch rút tiền này vào hàng chờ phê duyệt thủ công của Platform Manager (Sở Giao thông duyệt ở UC16) nhằm chống rửa tiền và gian lận.
17. **Bước B1.5:** Màn hình Mobile PWA hiển thị thông báo kết quả tương ứng cho hành khách:
    * *Với giao dịch dưới hạn mức (< 5tr):* Hiển thị thông báo "Rút tiền thành công! Số tiền đã được chuyển khoản tức thời vào tài khoản ngân hàng của bạn".
    * *Với giao dịch chờ duyệt (>= 5tr):* Hiển thị thông báo "Yêu cầu rút tiền của bạn đã được gửi thành công. Do số tiền lớn, yêu cầu đang được hệ thống khóa tạm và chờ Platform Manager phê duyệt để đảm bảo an ninh tài khoản".

###### Option B2: Company Manager gửi yêu cầu rút tiền quyết toán doanh nghiệp (Operator Wallet)
18. **Bước B2.1:** Company Manager đăng nhập Portal Doanh nghiệp, truy cập mục "Ví doanh nghiệp" -> chọn **"Rút tiền quyết toán"**.
19. **Bước B2.2:** Company Manager nhập số tiền rút khả dụng và nhập thông tin tài khoản ngân hàng nhận tiền thực tế của doanh nghiệp (Tên ngân hàng, Số tài khoản, Tên chủ tài khoản).
20. **Bước B2.3:** Company Manager nhấn nút **"Gửi yêu cầu rút tiền quyết toán"**.
21. **Bước B2.4:** Backend tiếp nhận yêu cầu và thực hiện giao dịch ACID kiểm duyệt:
    * So khớp số tiền yêu cầu rút nhỏ hơn hoặc bằng số dư ví doanh nghiệp khả dụng hiện tại.
    * Thực hiện trừ tạm thời số tiền yêu cầu rút khỏi số dư khả dụng hiện tại của Ví doanh nghiệp và đưa số tiền này vào cơ chế khóa tạm chờ duyệt để tránh hiện tượng rút đúp dòng tiền: `wallet.balance = balance - withdraw_amount`.
    * Khởi tạo giao dịch trong bảng `transactions` bên `ticket_db`: `transaction_type = 'WITHDRAW'`, `amount = withdraw_amount`, `status = 'PENDING_SETTLEMENT'`, `wallet_id = ID_Ví_Doanh_Nghiệp`.
22. **Bước B2.5:** Hệ thống Portal hiển thị thông báo "Yêu cầu rút tiền quyết toán của bạn đã được gửi thành công tới Sở Giao thông phê duyệt giải ngân" (Sở Giao thông phê duyệt ở UC16).

#### Luồng ngoại lệ (Exception Flows):
*   **Exc 10a - Chuyển khoản sai cú pháp nội dung trên Sepay:** Hành khách chuyển khoản thực tế nhưng tự ý thay đổi nội dung chuyển khoản khiến hệ thống Webhook không thể tự động bóc tách được `wallet_id`. Backend phát hiện dữ liệu lỗi, ghi nhận giao dịch vào hàng chờ kiểm tra thủ công ở trạng thái `status = 'MANUAL_REVIEW'` kèm logs chi tiết biến động số dư thực tế từ ngân hàng. Đồng thời, bắn cảnh báo tới Platform Manager trên Portal quản trị trung tâm để tiến hành kiểm tra, đối chiếu sao kê và cộng tiền tay thủ công cho khách.
*   **Exc 10b - Rút tiền vượt quá số dư khả dụng:** Hệ thống ném ra ngoại lệ `InsufficientFundsException`, từ chối tạo yêu cầu rút tiền quyết toán/rút tiền ví cá nhân và cảnh báo người dùng trên giao diện.
*   **Exc 10c - Gặp sự cố kết nối API Outbound Payout của ngân hàng (Trường hợp rút tự động):** Nếu cổng chuyển khoản Napas của Sepay/Ngân hàng bị lỗi downtime lúc nửa đêm, backend không nhận được callback thành công từ API. Hệ thống tự động chuyển giao dịch rút tiền của khách hàng sang trạng thái `PENDING_RETRY`, giữ nguyên phần tiền bị khóa trong ví khả dụng, và đẩy lệnh vào hàng đợi để scheduler tự động thực hiện lại lệnh (retry) khi API thông suốt, hoặc chuyển cho Platform Manager duyệt thủ công để đảm bảo không bị thất thoát tiền của khách hàng.

---

### UC16: Phê duyệt lệnh rút tiền quyết toán doanh nghiệp (Platform Manager)
*   **Mô tả:** Quản lý nền tảng (`PLATFORM_MANAGER` - Sở Giao Thông / Viettel VTS) thực hiện kiểm duyệt danh sách các yêu cầu rút tiền quyết toán doanh thu từ các công ty vận hành gửi lên, thực hiện giao dịch ngân hàng chuyển tiền thực tế bên ngoài hệ thống và nhấn phê duyệt xác nhận trên Portal để kết chuyển dòng tiền.

| Thuộc tính | Chi tiết đặc tả |
| :--- | :--- |
| **Mã Use Case** | **UC16** |
| **Tác nhân chính** | Quản lý nền tảng / Platform Manager |
| **Tiền điều kiện** | Tài khoản `PLATFORM_MANAGER` đã đăng nhập thành công vào Portal Quản trị Trung tâm. Có lệnh đề xuất rút tiền `PENDING_SETTLEMENT` từ các công ty vận hành thành viên. |
| **Hậu điều kiện** | Lệnh rút tiền được chuyển trạng thái sang `SUCCESS`, số tiền đã bị khóa trước đó trong Ví doanh nghiệp chính thức bị khấu trừ vĩnh viễn, gửi email thông báo hoàn tất kết chuyển quyết toán dòng tiền. |
| **Tác nhân kích hoạt** | Platform Manager chọn phê duyệt lệnh rút tiền trên Portal Quản trị Trung tâm. |

#### Luồng xử lý chính (Basic Flow):
1. **Bước 1:** Platform Manager truy cập danh mục "Phê duyệt Quyết toán tài chính" trên Portal.
2. **Bước 2:** Hệ thống hiển thị danh sách các lệnh rút tiền đang ở trạng thái `PENDING_SETTLEMENT` kèm theo các thông tin: Tên công ty yêu cầu, Số dư ví hiện tại, Số tiền đề xuất rút, Thông tin tài khoản ngân hàng nhận tiền doanh nghiệp.
3. **Bước 3:** Platform Manager tiến hành kiểm toán thông tin và thực hiện giao dịch ngân hàng chuyển khoản thực tế số tiền từ tài khoản Quỹ tổng hệ thống sang tài khoản ngân hàng của doanh nghiệp (qua Internet Banking ngân hàng ngoài hệ thống).
4. **Bước 4:** Sau khi chuyển khoản thành công và nhận được hóa đơn giao dịch ngân hàng, Platform Manager tải ảnh hóa đơn giao dịch lên hệ thống và nhấn nút **"Phê duyệt Giải ngân"** trên Portal.
5. **Bước 5:** Backend nhận yêu cầu và thực thi giao dịch ACID đồng bộ:
   * Cập nhật trạng thái giao dịch trong bảng `transactions` từ `PENDING_SETTLEMENT` sang **`SUCCESS`**.
   * Khấu trừ vĩnh viễn số tiền đã bị khóa ra khỏi số dư Ví doanh nghiệp của đơn vị vận hành đó.
   * Lưu liên kết ảnh hóa đơn ngân hàng đính kèm thông tin giao dịch phục vụ đối soát, thanh tra dòng tiền.
6. **Bước 6:** Hệ thống gửi email tự động đính kèm hóa đơn giải ngân, thông báo hoàn tất chuyển khoản quyết toán thành công cho Company Manager của đơn vị vận hành.

#### Luồng ngoại lệ (Exception Flows):
*   **Exc 16a - Platform Manager Từ chối phê duyệt (Reject Settlement Request):** Nếu phát hiện thông tin ngân hàng doanh nghiệp bị sai lệch hoặc số liệu quyết toán không hợp lệ, Platform Manager nhấn nút "Từ chối" và nhập lý do từ chối bắt buộc. Backend thực thi lệnh Rollback: Cộng trả lại số tiền đã khóa về số dư khả dụng của Ví doanh nghiệp, cập nhật trạng thái giao dịch thành **`FAILED`** kèm lý do chi tiết.

---

### UC20: Chạy ngầm đối soát & Phân chia doanh thu clearing (System Scheduler)
...
| **Hậu điều kiện** | Số dư Ví doanh nghiệp (`wallet.balance` của tài khoản `OPERATOR` tương ứng) của từng công ty vận hành được cộng tiền phân bổ chính xác. Tạo báo cáo bù trừ chi tiết trong `clearing_reports` và `clearing_details`. Toàn bộ chuyến đi được đánh dấu `processed_status = 'PROCESSED'`. |
...
6. **Bước 6:** **Thực thi giao dịch tài chính ACID trên Database:**
   * Tạo bản ghi báo cáo tổng hợp trong bảng `clearing_reports`.
   * Với từng công ty vận hành thành viên, tạo bản ghi chi tiết trong `clearing_details` with trạng thái `status = 'SETTLED'`.
   * **Cộng tiền trực tiếp:** Cập nhật tăng số dư Ví doanh nghiệp của công ty đó bên bảng `wallet` ở `auth_db`: `balance = balance + Total_Net_Settlement(i)`.
   * Ghi nhận một bản ghi giao dịch kế toán trong bảng `transactions` bên `ticket_db`: `transaction_type = 'CREDIT_CLEARING'`, `amount = Total_Net_Settlement(i)`, `wallet_id = ID_Ví_Doanh_Nghiệp_Công_Ty`.
...
### UC14: Quản lý Nhân viên ga & Phân lịch ca trực (Company Manager)
...
| **Tiền điều kiện** | Tài khoản `COMPANY_MANAGER` đã đăng nhập thành công và được gán chính xác mã doanh nghiệp `company_id`/`tenant_id`. |
...
2. **Bước 2:** Quản lý chọn hình thức tác vụ:
   ...
   * *Thiết lập ca trực (Roster):* Chọn nhân viên cụ thể, chọn nhà ga/trạm làm việc (`station_id`), chọn ngày trực và khung thời gian ca trực (Ca Sáng: 06:00 - 14:00, Ca Chiều: 14:00 - 22:00).
...
4. **Bước 4:** Backend nhận yêu cầu, thực hiện xác thực phân quyền và ghi nhận vào DB:
   ...
   * Ghi nhận lịch trực vào bảng `rosters`/`staff_shifts` ở trạng thái `PENDING` (chờ Staff vào mở ca hoạt động ở UC13).
...
#### Luồng rẽ nhánh (Alternative Flows):
*   **Alt 14b - Nhập hàng loạt nhân viên và phân lịch ca trực từ tệp (Bulk Import Staff & Rosters):**
    ...
    *   **Bước 5:** Backend thực hiện kiểm tra tính hợp lệ của từng dòng dữ liệu: kiểm tra định dạng email, số điện thoại, số CCCD; kiểm tra tính hợp lý của ngày trực và mã nhà ga; kiểm tra trùng lặp thông tin giữa các dòng trong tệp và đối sánh với CSDL hiện hữu (UC14 - Exc 14a).
    *   **Bước 6:** Backend thực hiện giao dịch ACID lô (Batch Transaction):
        *   *Nếu toàn bộ dữ liệu hợp lệ:* Hệ thống tự động tạo hàng loạt tài khoản `STAFF` gắn liền với `company_id` của doanh nghiệp, tự động gán vai trò `ROLE_STAFF`, lưu thông tin hồ sơ vào bảng `account_profile`, tạo lịch ca trực tương ứng trong bảng `staff_shifts` ở trạng thái `PENDING`, đồng thời kích hoạt Email Service tự động gửi mật khẩu tạm thời ngẫu nhiên cho từng nhân viên mới.
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 14a - Trùng lặp thông tin nhân sự:** Backend phát hiện Tên đăng nhập hoặc Email của nhân viên mới đã tồn tại trong CSDL. Hệ thống chặn lại, hiển thị thông báo lỗi chi tiết và yêu cầu Quản lý nhập lại thông tin khác.
*   **Exc 14c - Tệp nhập liệu không đúng cấu trúc mẫu hoặc trống rỗng (Alt 14b):** Tệp tải lên không khớp với cấu trúc cột quy định của hệ thống hoặc không chứa bất kỳ dòng dữ liệu nào. Hệ thống từ chối xử lý và báo lỗi: "Tệp nhập liệu không hợp lệ. Vui lòng sử dụng tệp mẫu tiêu chuẩn được cung cấp!".
*   **Exc 14d - Phát hiện lỗi dữ liệu logic khi nhập lô (Alt 14b):** Hệ thống phát hiện một hoặc nhiều dòng dữ liệu trong tệp vi phạm ràng buộc nghiệp vụ (trùng lặp email/CCCD, mã nhà ga không tồn tại, sai định dạng thông tin). Hệ thống hủy bỏ toàn bộ phiên giao dịch nhập lô, hiển thị cảnh báo: "Phát hiện lỗi dữ liệu tại [N] dòng trong tệp. Vui lòng tải về tệp báo cáo lỗi để sửa đổi trước khi thử lại!".

---

### UC23: Quản trị Tuyến trạm và Lưới nhà ga của đơn vị (Company Manager)
...
5. **Bước 5:** Backend tiếp nhận yêu cầu, thực thi xác thực quyền hạn Tenant Isolation (chỉ cho phép sửa tuyến trạm đính kèm `company_id` của chính đơn vị đó) và ghi nhận cập nhật vào các bảng `routes`, `stations`, `route_stations` của `ticket_db`.
...
#### Luồng rẽ nhánh (Alternative Flows):
*   **Alt 23b - Nhập hàng loạt mạng lưới tuyến đường và nhà ga/trạm dừng từ tệp (Bulk Import Routes & Stations):**
    ...
    *   **Bước 6:** Backend thực hiện kiểm tra toàn vẹn nghiệp vụ trên từng dòng dữ liệu: kiểm tra xem thứ tự các trạm dừng `station_order` của từng tuyến có liên tục tăng dần bắt đầu từ 1 và không trùng lặp hay không (UC23 - Exc 23a), xác thực tính chính xác của tọa độ GPS, kiểm tra xem mã tuyến hoặc tên trạm đã có trong hệ thống hay chưa, và thực thi kiểm tra phân vùng dữ liệu cô lập (Tenant Isolation - đảm bảo các trạm dừng đính kèm thuộc quyền sở hữu của `company_id` hiện tại).
    *   **Bước 7:** Backend thực hiện giao dịch ACID Batch (Lô):
        *   *Trường hợp 100% dữ liệu hợp lệ:* Ghi nhận hàng loạt tuyến mới vào bảng `routes`, các trạm dừng vào bảng `stations` và các bản ghi thứ tự liên kết vào bảng `route_stations` của `ticket_db`.
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 23a - Thứ tự trạm dừng bị trùng lặp hoặc nhảy quãng:** Nếu Quản lý cấu hình thứ tự trạm `station_order` bị trùng lặp (ví dụ: có hai trạm số 3) hoặc nhảy số không liên tục, backend phát hiện và từ chối cập nhật, hiển thị cảnh báo: "Thứ tự các trạm dừng trong tuyến không hợp lệ. Vui lòng sắp xếp thứ tự liên tục tăng dần!".
*   **Exc 23b - Tệp cấu hình tuyến trạm sai định dạng cấu trúc hoặc trống (Alt 23b):** Tệp do Quản lý tải lên không đúng với mẫu cột quy định hoặc không chứa dữ liệu. Hệ thống từ chối xử lý, báo lỗi: "Cấu trúc tệp không hợp lệ hoặc dữ liệu trống. Vui lòng kiểm tra lại tệp mẫu của hệ thống!".
*   **Exc 23c - Lỗi dữ liệu lưới tuyến trạm trong tệp nhập lô (Alt 23b):** Hệ thống phát hiện lỗi logic dữ liệu trong tệp (tọa độ GPS sai định dạng, trùng lặp mã tuyến, thứ tự trạm dừng vi phạm ràng buộc Exc 23a, hoặc vi phạm Tenant Isolation). Hệ thống từ chối import toàn bộ tệp, rollback và báo lỗi: "Dữ liệu lưới tuyến trạm trong tệp có lỗi. Vui lòng tải tệp báo cáo lỗi để kiểm tra và sửa đổi!".

---

### UC24: Thiết lập Cấu hình Biểu giá tuyến của đơn vị (Company Manager)
...
| **Hậu điều kiện** | Biểu giá cước mới được lưu thành công trên DB, tự động áp dụng cho các giao dịch quét soát vé của hành khách kể từ thời điểm cấu hình có hiệu lực. |
...
4. **Bước 4:** Backend tiếp nhận cấu hình biểu giá dưới định dạng JSON, chạy engine kiểm toán cước giả lập để đảm bảo tính đúng đắn và kiểm tra xem mức giá cấu hình có vượt quá **Khung giá trần** do Sở Giao thông ban hành (UC25) hay không.
5. **Bước 5:** Backend thực hiện lưu cấu hình vào bảng `fare_policies` thuộc `ticket_db` đính kèm `company_id`.
...
### UC15: Khởi tạo Tenant & Cấp tài khoản Company Manager (Platform Manager)
*   **Mô tả:** Quản lý nền tảng (`PLATFORM_MANAGER` - đại diện Sở Giao Thông / Viettel VTS) thực hiện khởi tạo hồ sơ, kích hoạt Tenant mới (các công ty vận hành tham gia mạng lưới như VinBus, Metro Hà Nội) và tự động sinh tài khoản Quản lý đơn vị (`COMPANY_MANAGER`) cùng Ví doanh nghiệp (`Operator Wallet`) để sẵn sàng vận hành kỹ thuật và hạch toán tài chính.
...
| **Hậu điều kiện** | Bản ghi Tenant mới được lưu thành công ở trạng thái `ACTIVE`, tài khoản `COMPANY_MANAGER` được cấp phát hoạt động và Ví doanh nghiệp (`Operator Wallet`) được tự động khởi tạo với số dư = 0đ. |
...
5. **Bước 5:** Backend tiếp nhận yêu cầu và thực thi giao dịch ACID đồng bộ:
   * Khởi tạo bản ghi Tenant mới vào bảng `tenants`/`companies`.
   * Tạo tài khoản quản trị trong bảng `accounts` với quyền `ROLE_COMPANY_MANAGER` liên kết với `tenant_id` mới.
   * Tự động khởi tạo Ví doanh nghiệp (`Operator Wallet`) tương ứng trong bảng `wallet` bên `auth_db` với số dư mặc định `balance = 0`.
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 15a - Trùng lặp MST hoặc Email đại diện doanh nghiệp:** Backend phát hiện mã số thuế hoặc email người đại diện đã được đăng ký trước đó trong hệ thống. Backend từ chối khởi tạo, giữ nguyên dữ liệu cũ và thông báo lỗi chi tiết cho Platform Manager trên Portal.

---

### UC25: Cấu hình Khung giá trần toàn hệ thống (Platform Manager)
...
| **Hậu điều kiện** | Khung giá trần mới được ghi nhận trong bảng cấu hình hệ thống `system_configs` toàn cục. Mọi thay đổi chính sách cước của các doanh nghiệp thành viên sau thời điểm này đều bị cưỡng bức đối chiếu theo khung giá mới. |
...
5. **Bước 5:** Backend tiếp nhận yêu cầu, xác thực tài khoản có thẩm quyền cấp cao (`ROLE_PLATFORM_MANAGER`), thực thi giao dịch ACID cập nhật các tham số tương ứng trong bảng `system_configs` thuộc CSDL trung tâm.
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 25a - Giá trần cấu hình âm hoặc không hợp lý:** Nếu Platform Manager nhập giá trị âm hoặc bằng 0, backend từ chối ghi nhận, hiển thị cảnh báo lỗi: "Mức giá trị trần cấu hình không hợp lệ. Vui lòng nhập số dương lớn hơn 0!".
...
### UC17: Khóa và Mở khóa tài khoản vi phạm khẩn cấp - Ban/Unban Account (Admin)
...
#### Luồng ngoại lệ (Exception Flows):
*   **Exc 17a - Admin tự ý khóa tài khoản của chính mình:** Backend chặn đứng hành vi tự khóa tài khoản của chính tài khoản `ADMIN` đang thao tác để bảo vệ hệ thống không bị lỗi mất kiểm soát cục bộ, hiển thị thông báo lỗi: "Không thể tự khóa tài khoản Admin đang đăng nhập!".

---

### UC18: Cấu hình phân quyền động - Dynamic RBAC Configuration (Admin)
...
### UC19: Giám sát kỹ thuật & Tra cứu System Logs (Admin)
...
* **100% Độc lập và Chi tiết:** Tài liệu được viết đầy đủ kịch bản cho từng Use Case riêng biệt từ UC01 đến UC27, không sử dụng liên kết tham chiếu hoặc nội dung tóm tắt rút gọn.
